...
describe('InstanceAiService — suspended run user revalidation', () => {
	...
	// Newly connected tools are additive, so a failed rebuild must not cost the user
	// the run — they just get the tools from their next message instead.
	it('resumes on the original agent when a connect-only rebuild fails', async () => {
		const service = createSuspendedRunResumeService();
	...
	// A freshly connected MCP server only contributes tools at agent construction,
	// so the resumed turn has to run on a rebuilt agent to use them right away.
