...
describe('LiveWebhooks', () => {
	...
	describe('webhook-phase isolate acquisition', () => {
		...
		const setupMocks = ({
			...
			webhookService.getWebhookMethods.mockResolvedValue([httpMethod]);
			// A plain literal, not `mock<INodeType>`: a mock proxy stubs every
			// property access truthy, so `getNodeParameters` sees `displayOptions`
			// on collection children and throws.
			nodeTypes.getByNameAndVersion.mockReturnValue({
				description: {
					displayName: 'Trigger',
					name: nodeType,
					...
					inputs: [],
					outputs: ['main'],
					// Load-bearing: the `Workflow` constructor drops undeclared
					// parameters, and the scan runs on that post-processed object.
		...
		it('still skips for the n8nOAuth2 authentication mode', async () => {
			// Webhook-phase credential resolution never uses this workflow's bridge:
			// `WebhookContext.getCredentials` passes no `connectionInputData`, so
			// `CredentialsHelper.getDecrypted` acquires its own isolate.
			const request = setupMocks({
		...
		it('still skips for a node that uses credentials', async () => {
			// Credentials live outside `node.parameters`, so the scan misses them —
			// safe because `CredentialsHelper.getDecrypted` acquires its own isolate.
