...
export class LiveWebhooks implements IWebhookManager {
	 ...
	 * nothing: every description field of the trigger resolves natively (see
	 * `webhookDescriptionFields` in n8n-workflow) and the node's own parameters
	 * contain no expressions. Anything not proven below acquires eagerly. If the
	 * skip is ever wrong, the first evaluation throws a hard `IsolateError` —
	 * loud, not a silent divergence.
	 */
