		...
		try {
			...
			if (USE_HOST_DOCKER) {
				...
				runner = runner
					...
					.withCommand([
						'iptables -w 5 -N DOCKER-USER >/dev/null 2>&1 || true; docker network rm runner-bridge >/dev/null 2>&1 || true; docker network create --driver bridge runner-bridge >/dev/null; exec /usr/local/bin/sandbox-runner',
					]);
			}
