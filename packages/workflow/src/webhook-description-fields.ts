 ...
 * backend they force the expression engine — and under
 * `N8N_EXPRESSION_ENGINE=vm` a V8 isolate — into every webhook request even
 * when the user authored no expression.
 *
...
export function fromFunction<P>(
	...
	return {
		...
		// The engine passes the same parameters object; P only narrows the
		// declaration site's view of it.
...
/**
 * Resolves a description field natively when a resolver is declared, the field
 * still carries the exact template that resolver was generated from, and the
 * node's parameters are static. The template check is load-bearing: object
 * spread copies the resolver map's symbol key, so a description built by
 * spreading another (Wait and GitHub reuse the Webhook node's) inherits
 * resolvers whose fields it may override — those must fall back to the engine.
 *
 * Callers must pass the workflow's own node (`workflow.nodes[name]`): the
 * `Workflow` constructor applies parameter defaults there, which resolvers rely
 * on just like the `$parameter` proxy.
 */
